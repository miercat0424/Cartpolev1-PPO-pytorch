...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, alpha, fc1_dims=256, fc2_dims=256, chkpt_dir="tmp/ppo"):     # Almost same as ActorNetwork
...
class Agent:
    ...
    def choose_action(self, observation):                                                       
        state       = T.tensor(np.array([observation]), dtype=T.float).to(self.actor.device)    # Observation of the Current State of the Env as Input
        # print(f"state :{state}")
        dist        = self.actor(state)                 # And then we'll get our distribution for choosing an 'action' from ActorNetwork by giving "State"
    ...
    def learn(self):
        for _ in range(self.n_epochs):
            ...
            for t in range(len(reward_arr)-1):                              # for each time step to len(reward)-1, 
                ...
                discount = 1.
                a_t      = 0
                for k in range(t, len(reward_arr)-1):                       # PPO Paper Algorithm 1, K Part (Multiple Epochs For Policy Updating)
